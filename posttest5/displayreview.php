    ...
    <div class="container mx-auto relative">
        ...
        <main>
            ...
            <table class="text-white">
                ...
                <?php
                // Query untuk mengambil semua data review dari tabel
                $sql = "SELECT id, email, judul, rating, review FROM reviews";
                $result = $conn->query($sql);

                // Jika ada hasil, tampilkan data
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td class='text-white'>" . htmlspecialchars($row['email']) . "</td>"; // Tambahkan kelas text-white
                        echo "<td class='text-white'>" . htmlspecialchars($row['judul']) . "</td>"; // Tambahkan kelas text-white
                        echo "<td class='text-white'>" . htmlspecialchars($row['rating']) . "/5</td>"; // Tambahkan kelas text-white
                        echo "<td class='text-white'>" . htmlspecialchars($row['review']) . "</td>"; // Tambahkan kelas text-white
                        echo "<td>
                    ...
                    echo "<tr><td colspan='5' class='text-white'>No reviews found.</td></tr>"; // Tambahkan kelas text-white
                }
                ?>
            </table>
        </main>
